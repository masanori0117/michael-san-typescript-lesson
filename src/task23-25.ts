...
const halfNumbers: number[] = numbers.map(number => number / 2)
console.log(halfNumbers)

console.log("2. 文字列配列 `['a', 'b', 'c', 'd', 'e']` の各要素を `'_'` で囲んだ新しい配列を作成してください（例：`'_a_'`）。")
const letters: string[] = ['a', 'b', 'c', 'd', 'e'];
const newLetters: string[] = letters.map(letter => `_${letter}_`);
console.log(newLetters)

console.log("3. 数字の配列 `[1, 2, 3, 4, 5]` を使って、各要素を文字列に変換した新しい配列を作成してください。")
numbers = [1, 2, 3, 4, 5];
const numberssToStrings: string[] = numbers.map(number => number.toString());
...
numbers = [1, 2, 3, 4, 5, 6]
let totalNumber: number = numbers.reduce((total, number) => total + number, 0);

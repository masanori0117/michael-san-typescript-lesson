...
const newLetters = letters.map(letter => `_${letter}_`);
console.log(newLetters)

console.log("3. 数字の配列 `[1, 2, 3, 4, 5]` を使って、各要素を文字列に変換した新しい配列を作成してください。")
numbers = [1, 2, 3, 4, 5];
const numberssToStrings = numbers.map(number => number.toString());
...
numbers = [1, 2, 3, 4, 5, 6]
let totalNumber = numbers.reduce((total, number) => total + number, 0);
